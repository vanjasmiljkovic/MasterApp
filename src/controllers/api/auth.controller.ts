...
export class AuthController {
    ...
    @Post('administrator/refresh') //POST http://localhost:3000/auth/administrator/refresh
    async administratorTokenRefresh(@Req() req: Request, @Body() data: AdministratorRefreshTokenDto): Promise <LoginInfoDto | ApiResponse> {
        ...
        if (!administratorToken){ //ne postoji aktivni token
        ...
        const now = new Date();
        const expiresDate = new Date(administratorToken.expiresAt);
